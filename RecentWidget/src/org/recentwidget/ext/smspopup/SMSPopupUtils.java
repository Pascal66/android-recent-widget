...
public class SMSPopupUtils {
	...
	public static String getPersonIdFromPhoneNumber(Context context,
		...
		Cursor cursor = context.getContentResolver().query(
				Uri.withAppendedPath(Contacts.Phones.CONTENT_FILTER_URL,
						address), new String[] { Contacts.Phones.PERSON_ID },
				null, null, null);
		if (cursor != null) {
			try {
				if (cursor.getCount() > 0) {
	...
	public static long getThreadIdFromAddress(Context context, String address) {
		...
		Cursor cursor = context.getContentResolver().query(uriBuilder.build(),
				new String[] { SMS_ID }, null, null, null);
		if (cursor != null) {
			try {
	 ...
	 */
	public static void setMessageRead(Context context, long messageId,
			int messageType) {
		...
		if (messageId > 0) {
			...
			if (SmsMmsMessage.MESSAGE_TYPE_MMS == messageType) {
				messageUri = Uri.withAppendedPath(MMS_CONTENT_URI, String
						.valueOf(messageId));
			} else if (SmsMmsMessage.MESSAGE_TYPE_SMS == messageType) {
				messageUri = Uri.withAppendedPath(SMS_CONTENT_URI, String
						.valueOf(messageId));
			} else {
				return;
			}
	...
	public static long findMessageId(Context context, long threadId,
			long _timestamp, int messageType) {
		long id = 0;
	...
	public static void deleteMessage(Context context, long messageId,
		...
		if (messageId > 0) {
			...
			if (SmsMmsMessage.MESSAGE_TYPE_MMS == messageType) {
				deleteUri = Uri.withAppendedPath(MMS_CONTENT_URI, String
						.valueOf(messageId));
			} else if (SmsMmsMessage.MESSAGE_TYPE_SMS == messageType) {
				deleteUri = Uri.withAppendedPath(SMS_CONTENT_URI, String
						.valueOf(messageId));
			} else {
				return;
			}
	...
	/**
		 * 
		 */
	public static Intent getSmsToIntentFromThreadId(Context context,
			long threadId) {
		...
		if (threadId > 0) {
			...
			popup.setData(Uri.withAppendedPath(THREAD_ID_CONTENT_URI, String
					.valueOf(threadId)));
		} else {
			return getSmsIntent();
		}
	...
	*/
	...
	public static int getUnreadSmsCount(Context context, long timestamp) {
		...
		Cursor cursor = context.getContentResolver().query(
				SMS_INBOX_CONTENT_URI, new String[] { SMS_ID },
				UNREAD_CONDITION, null, null);
	...
	public static int getUnreadMmsCount(Context context) {
		...
		Cursor cursor = context.getContentResolver().query(
				MMS_INBOX_CONTENT_URI, new String[] { SMS_ID },
				UNREAD_CONDITION, null, null);
		...
		if (cursor != null) {
			try {
				...
				if (count > 0) {
					...
					SmsMmsMessage smsMessage = new SmsMmsMessage(context,
	...
	public static SmsMmsMessage getMmsDetails(Context context,
		...
		Cursor cursor = context.getContentResolver().query(
				...
				UNREAD_CONDITION, null, SORT_ORDER);
		...
		if (cursor != null) {
			try {
				...
				if (count > 0) {
					...
					SmsMmsMessage mmsMessage = new SmsMmsMessage(context,
							address, subject, timestamp, threadId, count,
	...
	private static String getDisplayName(Context context, String email) {
		...
		Cursor cursor = context.getContentResolver().query(
				...
				Contacts.ContactMethods.DATA + " = \'" + email + "\'", null,
				null);
	...
	public static String formatTimestamp(Context context, long timestamp) {
		String HOURS_24 = "24";
		String hours;
		hours = Settings.System.getString(context.getContentResolver(),
